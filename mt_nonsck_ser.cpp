...
struct dir_basket dirbsk[NUM_DIRECTORY];
...
static struct thread_status status[NUM_THREADS];

char rootdir[PATH_MAX] = "rootdir";

pthread_mutex_t mutex_ser;

////// Life gonna hell.... All is global variables
...
int idx_pool;
//static int idx_and_sfd[NUM_THREADS][2];
//////////// ////////////

void *server_func(void *idx_pool);
...
int main (int argc, char *argv[])
{
  ...
  pthread_mutex_init(&mutex_ser, NULL);
  ...
  for (int i=0;i<NUM_THREADS;i++) {
    listen(sockfd, 100);
    pthread_create(&worker[i], NULL, server_func, NULL);
  }
  ...
  /*while (1) {
    ...
    //cout << "[main1] Get into main " << idx_pool << endl;
    pthread_mutex_lock(&mutex_ser);
		for (int i=0;i<100;i++) {
			if (status[i].done) {
        //cout << "Get in here, i : " << i << ", tid : " << *pool[i] << endl;
        status[i].tid = 0;
			  delete pool[i];
        status[i].done = false;
      }
    ...
    aa = sizeof(cli_addr);
	  newsckfd = :: accept(sockfd, (struct sockaddr *) &cli_addr, (socklen_t*)&aa);
    ...
    pthread_create(worker, NULL, server_func, NULL);
    pthread_detach(*worker);

    pool[idx_pool] = worker;
		status[idx_pool].tid = *worker;
    status[idx_pool].sfd = newsckfd;
    
    //index_pool[idx_pool] = i;
    //cout << "[main2] idx : " << idx_pool << endl << endl;
    idx_pool = (idx_pool+1)%100;
    //for (int i=0;i<NUM_THREADS;i++)
...
void *server_func(void *idx_and_sfd) 
{
  //int newsockfd = ((int*)idx_and_sfd)[0];
  //int idx = ((int*)idx_and_sfd)[1];
  ...
  /*pthread_mutex_lock(&mutex_ser);
  for (int i=99; i >= 0; i=(i+99)%100) {
    if (status[i].tid == pthread_self()) {
      idx = i;
      break;
    }
  }
  ...
  //newsockfd = status[idx].sfd;
  cout << "newsockfd : " << newsockfd;
  cout << ", tid : " << pthread_self() << endl;
  
  while(1) {
	//printf("[func1] idx : %d, sfd : %d\n", idx, newsockfd);
		...
		/////// Case of readdir function
			...
			if (::read(newsockfd, buff_work, BLOCK_SIZE) > 0) {
				...
				fullpath(fpath, obj_releasedir_recv.path().c_str());
			}
			else {
			...
			ret = -1;
			for (int i=0;i<NUM_DIRECTORY;i++) {
				if (dirbsk[i].dp != NULL && dirbsk[i].path == fpath) {                             
					ret = closedir(dirbsk[i].dp);
					dirbsk[i].dp = NULL;
					dirbsk[i].path = "";
					break;
				}
			...
			while (size > 0) {
				if (size >= BLOCK_SIZE) {
					...
					ret = pread(fd, read_buf, size, offset);
				}
